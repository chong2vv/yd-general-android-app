...
class UserUpdateViewModel : BaseRecyclerViewModel() {
            ...
            if (uid > 0) {
                ...
                user?.let {
                    val viewDataList: List<BaseViewData<*>> = listOf<BaseViewData<*>>(
                        UserUpdateItemViewData(UserUpdateBean("名字",
                            user.username.toString(), UserUpdateType.USER_NAME)),
                        UserUpdateItemViewData(UserUpdateBean("简介",
                            user.user_desc.toString(), UserUpdateType.USER_DESC)),
                        UserUpdateItemViewData(UserUpdateBean("年龄",
                            user.age.toString(), UserUpdateType.USER_AGE)),
                        UserUpdateItemViewData(UserUpdateBean("地址",
                            user.address.toString(), UserUpdateType.USER_ADDRESS)),
                        UserUpdateItemViewData(UserUpdateBean("标签",
